...
import com.dianping.shop.entity.ShopReview;
import com.dianping.shop.mapper.ShopReviewMapper;
import com.dianping.shop.service.ShopReviewService;
...
public class ShopReviewServiceImpl extends ServiceImpl<ShopReviewMapper, ShopReview> implements ShopReviewService {
    ...
    //跨服务查用户：分库后 user 表在 dianping_user，走 Feign 调 user-service
    @Autowired
    ...
    public Result updateReview(ShopReview shopReview) {
        ...
        //修复：原代码用 userDTO.getId() 当评论id查，改为按评论id查
        ShopReview oldReview=getById(shopReview.getId());
        if(oldReview==null){
            return Result.fail("评论不存在");
        }
    ...
    public Result saveReview(ShopReview shopReview) {
        ...
        //校验内容
        if(shopReview.getContent()==null||shopReview.getContent().isBlank()){
    ...
    private void updateShopScore(Long shopId){
        ...
        //修复：原代码 queryById 只查不写，改为走带缓存删除的 update
