...
import com.dianping.shop.service.ShopTypeService;
...
import static com.dianping.common.util.RedisContants.CACHE_SHOP_TYPE_KEY;
...
public class ShopTypeServiceImpl extends ServiceImpl<ShopTypeMapper, ShopType> implements ShopTypeService {
    ...
    public Result queryList() {
        ...
        List<String> shopTypeJsonList=new ArrayList<>();
        for(ShopType shopType:tmp){
            String jsonStr=JSONUtil.toJsonStr(shopType);
            shopTypeJsonList.add(jsonStr);
        }
        stringRedisTemplate.opsForList().leftPushAll(CACHE_SHOP_TYPE_KEY,shopTypeJsonList);
        //把最终查询到的商户分类信息返回到前端
        return Result.ok(tmp);
    }
}
