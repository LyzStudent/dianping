-- ================================================
-- 库：dianping_shop（独立分库）
-- 说明：每个微服务一个独立数据库，表之间不再跨库建外键
-- 执行方式：Navicat/命令行直接运行本文件即可建库+建表+导入数据
-- ================================================
CREATE DATABASE IF NOT EXISTS dianping_shop DEFAULT CHARACTER SET utf8mb4;
USE dianping_shop;
...
-- ----------------------------
-- Table structure for tb_shop_review（店铺评价，项目新加的模块）
-- ----------------------------
...
CREATE TABLE `tb_shop_review`  (
                                  ...
                                  `rating` tinyint(1) UNSIGNED NOT NULL DEFAULT 5 COMMENT '评分1~5星',
                                  `content` varchar(1024) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '评价内容',
                                  `images` varchar(1024) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '评价图片，多张以","隔开',
...
CREATE TABLE `tb_banner`  (
                              ...
                              `image` varchar(512) NOT NULL DEFAULT '' COMMENT '图片URL，可留空用渐变',
                              `link` varchar(255) NOT NULL DEFAULT '' COMMENT '跳转：/seckill 或 http(s)',
